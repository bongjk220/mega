        ...
        if (productSearch) {
            productSearch.addEventListener('input', function (e) {
                ...
                cards.forEach(card => {
                    ...
                    const description = card.querySelector('.prod-sub').textContent.toLowerCase();
...
function updateWordCloud(keywords) {
    ...
    wordCloud.innerHTML = keywords.map(keyword => {
        const fontSize = Math.max(12, Math.min(32, keyword.frequency / 20));
        const fontWeight = keyword.frequency > 50 ? '800' : keyword.frequency > 30 ? '700' : '500';
        const color = keyword.frequency > 50 ? '#1a7a4a' : keyword.frequency > 30 ? '#2d6a4f' : '#999';

        return `<span style="font-size:${fontSize}px;font-weight:${fontWeight};color:${color};cursor:default">${keyword.name}</span>`;
    }).join('');
}
    ...
    return `
        <div class="order-item" onclick="selectOrder(${order.id})" data-order-id="${order.id}">
            ...
            <div class="order-foot">
                ...
                <span class="order-price" style="color:#2d9a5e">${formatPrice(order.totalPrice)}</span>
            </div>
        </div>
    `;
}
        ...
        <div class="invoice-card">
            ...
            <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;padding-top:10px;border-top:1px solid #f0f2f0">
                ...
                <div style="display:flex;gap:48px;font-size:16px;font-weight:700;margin-top:12px;padding-top:12px;border-top:1px solid #e8ece8;align-items:center">
                    ...
                    <span class="total-pill">${formatPrice(order.totalPrice)}</span>
                </div>
            </div>
...
function updatePaginationControls(page, productCount) {
    ...
    // Create custom pagination arrows
    let paginationHTML = '';
    
    // Previous arrow image - positioned at left center
    if (page > 0) {
        paginationHTML += `<div class="pagination-left">
            <div class="pagination-arrow" onclick="goToPage(${page - 1})">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#2d9a5e" stroke-width="2">
    ...
    // Next arrow image - positioned at right center
    if (hasMore) {
    ...
    // Insert pagination arrows directly into the content area
    const content = document.querySelector('.content');
    if (content) {
        // Remove existing pagination if present
        const existingLeft = content.querySelector('.pagination-left');
        const existingRight = document.querySelector('.pagination-right');
...
function confirmAutoOrder() {
    ...
    const reasonTextarea = document.getElementById('auto-order-reason');
    const reason = reasonTextarea ? reasonTextarea.value : 'Auto order changed by admin';
